		...
		for (int i = 0; i < message.length; i++) {
			...
			for (int j = 0; j < item.length; j++) {
				if (chank.equals(item[j])) {
					for (int k = 0; k < itemBox.length; k++) {
						if (chank.equals(item[k])) {
							itemBox[k] = chank;
						}
					}
				}
